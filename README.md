...
<head>
  ...
  <style>
    :root{
      ...
      --brand:var(--purple);
      --brand-2:#5d0b8c;
      /* Pastel bar colors per character */
      --c-uri:#a4c8ff;
      --c-buni:#b6f7d0;
      --c-marsy:#d9b79a;
      --c-veny:#ffc49a;
    ...
    *{box-sizing:border-box}
    body{
      ...
      min-height:100vh; margin:0; color:var(--ink); line-height:1.5;
      background:url('https://i.ibb.co/wrSgCpMc/9dbcf0b1c7021ed2a22361ad29ab98fe.jpg') center/cover fixed no-repeat;
      overflow-x:hidden;
    }
    ...
    @keyframes drift{ 0%,100%{ transform:translateY(0) rotate(0deg) scale(1) } 50%{ transform:translateY(-16px) rotate(2deg) scale(1.03) } }

    header{ text-align:center; padding:32px 16px 12px; position:relative; z-index:3; }
    h1{ margin:0; color:#fff; font-size:32px; text-shadow:0 2px 6px rgba(0,0,0,.6); }
    p.lead{ margin:8px 0 0; color:#fff; text-shadow:0 2px 6px rgba(0,0,0,.6);}    

    .wrap{ max-width: 900px; margin: 0 auto; padding: 0 16px 60px; position:relative; z-index:4; }
    .card{ background:var(--card); border:1px solid rgba(0,0,0,0.06); border-radius:16px; padding:18px; box-shadow:0 8px 30px rgba(0,0,0,0.1); }
    .stack{ display:grid; gap:16px; }

    .question{ border:1px solid rgba(0,0,0,0.06); border-radius:14px; padding:14px; background:var(--beige) }
    .question h3{ margin:0 0 8px; font-size:16px; color:var(--purple); }
    label{ display:block; padding:6px 8px; border-radius:10px; cursor:pointer; background:#fff8; }
    label:hover{ background:rgba(255,255,255,0.9) }
    ...
    #result{ margin-top:12px; background:var(--beige); border:1px solid rgba(0,0,0,0.08); border-radius:16px; padding:16px }
...
<body>
  ...
  <script>
    // 1) Characters & direct image links
    ...
    // 2) Render floating background images (all characters)
    const BG_ORDER = ["Pluty","Sunny","Moony","Uri","Marsy","Veny","Buni"];
    function attachImage(imgEl, sources){
      let i=0; function next(){ if(i>=sources.length){ imgEl.remove(); return; } imgEl.src=sources[i++]; }
      imgEl.addEventListener('error', next); next();
    }
    (function renderBackground(){
      const bg = document.getElementById('bgDecor');
      bg.innerHTML = '';
      BG_ORDER.forEach((name,idx)=>{
        const srcs = IMAGE_SOURCES[name]||[]; if(!srcs.length) return;
        const img=document.createElement('img');
        img.alt=name; img.className=`float f${idx+1}`; attachImage(img, srcs); bg.appendChild(img);
      });
    })();
    ...
    function computeScores(answers){
      ...
      for(const q in answers){
        const a = answers[q];
        if(POINTS[q] && POINTS[q][a]){
          for(const c in POINTS[q][a]) totals[c]+=POINTS[q][a][c];
        }
      }
    ...
    function renderResult({pct,winner}){
      ...
      winImg.alt = winner; winImg.width=72; winImg.height=72; winImg.style.borderRadius='50%';
      winImg.style.clipPath = 'circle(50% at 50% 50%)';
      attachImage(winImg, IMAGE_SOURCES[winner]||[]);

      const bars = CHARACTERS.map(c=>{
        const v = pct[c]; const w = Math.max(2, Math.min(100, v)).toFixed(1);
        const color = COLOR[c] || '#ddd';
        return `
          <div class="rowline">
      ...
      head.appendChild(winImg); head.appendChild(title);
    ...
    form.addEventListener('submit', (e)=>{
      ...
      const out = computeScores(answers); renderResult(out);
    });

    resetBtn.addEventListener('click', ()=>{ form.reset(); resultBox.innerHTML=''; });
  </script>
</body>
</html>
